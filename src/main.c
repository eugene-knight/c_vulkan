...
#define SYVK(NAME) SY_Vulkan_##NAME
    ...
    VkDebugUtilsMessengerCreateInfoEXT *createInfo)
{
    ...
    createInfo->pfnUserCallback = SYVK (DebugCallback);
...
SY_Vulkan_CheckValidationLayerSupport ()
{
    ...
    VkLayerProperties *layerProperties = SY_TALLOCATE_ (
        SY_DEFAULT_ALLOCATOR_PTR, VkLayerProperties, layerCount);
...
SY_Vulkan_InstanceInitialize (SY_Vulkan_Context *pContext)
{
    ...
    if (SY_ENABLE_VALIDATION_LAYERS)
        {
            extensionCount += 1;
            SY_IGNORE SY_Vector_Initialize (&extensionNames, extensionCount,
                                            SY_DEFAULT_ALLOCATOR_PTR);
            for (u32 i = 0; i < sdlExtensionCount; i++)
                {
                    SY_Vector_PushbackUnsafe (&extensionNames,
                                              sdlExtensionNames[i]);
                }
            SY_Vector_PushbackUnsafe (&extensionNames,
    ...
    if (SY_ENABLE_VALIDATION_LAYERS)
        {
            ...
            SYVK (PopulateDebugUtilsMessenger (&debugInfo));
    ...
    SY_Vector_Release (&extensionNames, SY_DEFAULT_ALLOCATOR_PTR);
    return result;
}
...
SY_Vulkan_DebugUtilsMessengerEXTInitialize (SY_Vulkan_Context *pContext,
                                            SYVK (Functions) * pFunctions)
{
    SY_Result errorCode = { .domain = SY_DOMAIN_SUCCESS, .code = 0 };
    VkDebugUtilsMessengerCreateInfoEXT createInfo;
    SYVK (PopulateDebugUtilsMessenger) (&createInfo);
...
enum
{
    SY_VULKAN_QUEUE_GRAPHICS_BIT = 0x01,
};

typedef struct SY_Vulkan_QueueFamilyIndices
{
    u8 graphicsFamily;
    u8 optionalMask;
} SY_Vulkan_QueueFamilyIndices;

bool
SY_Vulkan_QueueFamilyIndices_IsComplete (
    const SY_Vulkan_QueueFamilyIndices *const pIndices)
{
    return (pIndices->optionalMask & SY_VULKAN_QUEUE_GRAPHICS_BIT);
}

SY_Result
SY_Vulkan_FindQueueFamilies (SY_Vulkan_QueueFamilyIndices *pIndices,
                             VkPhysicalDevice device)
{
    ...
    vkGetPhysicalDeviceQueueFamilyProperties (device, &queueFamilyCount,
                                              nullptr);
    VkQueueFamilyProperties *queueFamilyProperties = SY_TALLOCATE_ (
        SY_DEFAULT_ALLOCATOR_PTR, VkQueueFamilyProperties, queueFamilyCount);
    vkGetPhysicalDeviceQueueFamilyProperties (device, &queueFamilyCount,
            ...
            if (queueFamilyProperties[i].queueFlags & VK_QUEUE_GRAPHICS_BIT)
                {
                    ...
                    pIndices->optionalMask |= SY_VULKAN_QUEUE_GRAPHICS_BIT;
                }
            if (SYVK (QueueFamilyIndices_IsComplete (pIndices)))
                {
                    break;
                }
            i += 1;
        }
    SY_DEALLOCATE_ (SY_DEFAULT_ALLOCATOR_PTR, &queueFamilyProperties[0]);
    return result;
}

bool
SY_Vulkan_IsDeviceSuitable (VkPhysicalDevice physicalDevice)
{
    SY_Vulkan_QueueFamilyIndices indices = { 0 };
    SYVK (FindQueueFamilies) (&indices, physicalDevice);
    return SYVK (QueueFamilyIndices_IsComplete) (&indices);
}

SY_Result
SY_Vulkan_PickPhysicalDevice (SY_Vulkan_Context *pContext)
{
    SY_Result result = { .domain = SY_DOMAIN_SUCCESS, .code = 0 };
    u32 deviceCount = 0;
    ...
    vkEnumeratePhysicalDevices (pContext->instance, &deviceCount,
                                &physicalDevices[0]);

    for (u32 i = 0; i < deviceCount; i++)
        {
            if (SYVK (IsDeviceSuitable) (physicalDevices[i]))
                {
    ...
    if (pContext->physicalDevice == nullptr)
        {
            ...
            result.domain = SY_DOMAIN_VULKAN;
            result.code = SY_RESULT_PLACEHOLDER;
            return result;
        }
    SY_DEALLOCATE_ (SY_DEFAULT_ALLOCATOR_PTR, &physicalDevices[0]);
...
main ()
{
    SYVK (Context) context = { 0 };
    SYVK (Functions) functions = { 0 };
    ...
    SY_IGNORE SYVK (InstanceInitialize (&context));
    SY_IGNORE SYVK (FunctionPointersInitialize) (&context, &functions);
    SY_IGNORE SYVK (DebugUtilsMessengerEXTInitialize) (&context, &functions);
    SY_IGNORE SYVK (PickPhysicalDevice) (&context);
    SY_IGNORE SYVK (FindQueueFamilies) (&indices, context.physicalDevice);
    ...
    if (SY_ENABLE_VALIDATION_LAYERS)
        {
            SYVK (DebugUtilsMessengerEXTRelease) (&context, &functions);
        }
    SYVK (InstanceRelease) (&context);
